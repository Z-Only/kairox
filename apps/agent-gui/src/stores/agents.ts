...
export const runningAgents = computed(() =>
  [...agentState.agents.values()].filter((a) => a.status === "running")
);

/** Group agents by their role. */
export const agentsByRole = computed(() => {
...
export const agentCountsByRole = computed(() => {
...
export function applyAgentEvent(payload: EventPayload) {
  switch (payload.type) {
    case "AgentSpawned": {
      ...
      agentState.agents.set(payload.agent_id, {
        id: payload.agent_id,
        role,
    ...
    case "AgentIdle": {
      const agent = agentState.agents.get(payload.agent_id);
      if (agent) {
        agent.status = "idle";
    ...
    case "AgentTaskFailed": {
      // Mark the assigned agent as failed if one is still running for this task
      for (const agent of agentState.agents.values()) {
    ...
    case "AgentTaskCompleted": {
      // Mark the assigned agent as completed if one is still running for this task
      for (const agent of agentState.agents.values()) {
        if (agent.taskId === payload.task_id && agent.status === "running") {
    ...
    case "TaskRetried": {
      // Reset the agent assigned to this task back to "running"
      for (const agent of agentState.agents.values()) {
...
export function agentLabel(agentId: string): string {
  ...
  // Count how many agents of the same role were spawned before this one
  // to get a sequential number
  const sameRoleAgents = [...agentState.agents.values()]
