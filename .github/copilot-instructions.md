...
After regenerating `Cargo.lock`, verify that only workspace package versions changed. If registry package versions, checksums, or sources drift, restore the lockfile from the latest `main` and run `cargo update -w`.
