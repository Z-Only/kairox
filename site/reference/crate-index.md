...
| What           | Detail                                                                                                                                   |
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| Repo path      | [`crates/agent-sdk`](https://github.com/Z-Only/kairox/tree/main/crates/agent-sdk)                                                        |
| Purpose        | Embeddable SDK exposing the Kairox runtime as a programmatic API for external harnesses, CI/CD pipelines, and custom UIs.                |
| Key types      | `KairoxSdk`, `SdkBuilder`, `SdkSession`, `MessageStream`, `StreamEvent`, `CollectedResponse`, `SdkHook` trait, `HookAction`, `SdkConfig` |
| Depended on by | External consumers that embed the Kairox runtime.                                                                                        |
...
| `agent-sdk`       | Small                | Public API is new and evolving; builder + stream patterns are stable.                                                          |
