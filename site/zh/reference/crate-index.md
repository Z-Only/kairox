...
内置 tool 包括:`shell.exec`、`fs.read`、`fs.write`、`fs.list`、`patch.apply`、`search.ripgrep`、`monitor.start`、`monitor.list` 和 `monitor.stop`。`PolicyEngine::decide(PolicyRisk)` 返回 `PolicyDecision`:`Allowed`、`DeniedBySandbox { reason }` 或 `NeedsApproval { reason }`;runtime 把后者转换成 permission event。旧的单轴 `PermissionMode` 枚举已在 v0.31.0 端到端移除。详情见 [Permissions & Tools](../concepts/permissions-and-tools)。
