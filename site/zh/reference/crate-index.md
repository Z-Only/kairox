...
| 关键类型 | `Tool`、`ToolRegistry`、`PolicyEngine`、`ApprovalPolicy`、`SandboxPolicy`、`PolicyDecision`、`PolicyRisk`、`ApprovalReason`、`ShellExecTool`、`PatchApplyTool`、`RipgrepSearchTool` |
| 被谁依赖 | `agent-runtime`、`agent-mcp`(经由 `McpToolAdapter`)                                                                                                                                 |

内置 tool 包括:`shell`、`fs.read`、`fs.write`、`fs.list`、`patch`、`search`。`PolicyEngine::decide(PolicyRisk)` 返回 `PolicyDecision`:`Allowed`、`DeniedBySandbox { reason }` 或 `NeedsApproval { reason }`;runtime 把后者转换成 permission event。旧的单轴 `PermissionMode` 枚举已在 v0.31.0 端到端移除。详情见 [Permissions & Tools](../concepts/permissions-and-tools)。
...
| 用途     | MCP 客户端、transport(stdio + SSE)、生命周期状态机、健康检查、协议类型,以及 marketplace 目录。                  |
