...
- 内置工具：`shell`、`fs.read`、`fs.write`、`fs.list`、`patch`、`search`，以及用于列出和停止后台进程的 monitor registry 工具。
...
- **TUI** 基于 ratatui：三栏布局、流式 chat、monitor stream item、trace 面板、permission overlay、命令面板、settings / marketplace overlay，以及 monitor list / stop 命令。
- **GUI** 基于 Tauri 2 + Vue 3：持久化 session、任务图、可搜索的 trace 时间线、memory 浏览器、monitor chat stream 渲染与 trace-store 处理、内联 permission 流程、按 session 配置 `ApprovalPolicy` 与 `SandboxPolicy` 的选择器、可调整宽度的工作台侧栏、项目 workspace、覆盖 model / agent / MCP / skill / plugin / hook / 指令的设置 tab，以及用于 monitor list / stop 的 Tauri IPC 控制。
- Tauri 2 的自动更新已经接入 GitHub Releases。

### 可扩展性

- 原生 **skill** 支持 workspace / user / session 三种 scope；支持从 SkillHub 安装。
- **Plugin** 通过 manifest 一次性打包 skill、tool、hook 与 MCP server；plugin 提供的 skill 在发现时带上命名空间；manifest 还包含 permission hints 与 compatibility metadata，供 marketplace 展示和后续安装策略使用。
...
- `kairox-eval` headless harness 提供确定性的 smoke、tool-call、compaction 与 tag-filter 场景；expectation 覆盖必需和禁止出现的 event type，以及 tool invocation 限制。
...
- 持续扩展 MCP 生态：更多 transport、更丰富的发现机制，以及 server health diagnostics。
...
- 将 `kairox-eval` 扩展为真正的 headless/CI 模式，支持 fixture、脚本化 prompt、退出码和 JSON report。
- 在 TUI 和 GUI 中支持用户可扩展的 slash command、output style 与 statusline 定制。
- 可观测与回放工具：结构化 trace export、基于 `EventStore` 的 event replay，以及脱敏 diagnostics bundle。
