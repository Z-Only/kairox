...
- 五种 permission 模式：`ReadOnly`、`Suggest`、`Agent`、`Autonomous`、`Interactive`。
...
- **GUI** 基于 Tauri 2 + Vue 3：持久化 session、任务图、可搜索的 trace 时间线、memory 浏览器、内联 permission 流程、按 session 配置 permission 模式、可调整宽度的工作台侧栏、项目 workspace，以及覆盖 model / agent / MCP / skill / plugin / hook / 指令的设置 tab。
...
- 可配置的 Agent 覆盖项按角色生效（model、permission 模式、skill、tool 白名单）。
