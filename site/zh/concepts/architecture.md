...
| Crate             | 角色                                                                                                                                         | 关键类型                                                                                                                                |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| **agent-runtime** | 编排 agent loop、session 生命周期、context budgets、compaction、模型切换、可配置的 agent 设置、multi-agent 策略、MCP 生命周期、permissions。 | `LocalRuntime<S, M>`、`PlannerAgent`、`WorkerAgent`、`ReviewerAgent`、`AgentStrategy`、`DagExecutor`、`TaskGraph`、`McpServerManager`。 |
...
| **agent-tools**   | 工具注册表、permission engine、内置工具(`shell`、`fs.read`、`fs.write`、`fs.list`、`patch`、`search`),以及 MCP-tool 适配器。                 | `ToolRegistry`、`PermissionEngine`、`Tool` trait、`PermissionMode`、`ToolRisk`、`McpToolAdapter`。                                      |
| **agent-mcp**     | MCP(Model Context Protocol)客户端、stdio + SSE 传输、server 生命周期、discovery 缓存、marketplace 目录(内置 + 远端来源)。                    | `McpClient`、`Transport` trait、`StdioTransport`、`SseTransport`、`ServerLifecycle`、`McpServerDef`、`CatalogEntry`。                   |
...
本页是这套系统的地图。它不会展开 runtime 在每一 turn 内的行为、memory 协议、permission 模式或扩展面。每一项都在本节中有专门的一页。下一步,从 [Runtime & Sessions](./runtime-and-sessions) 开始,看看每个 prompt 背后发生了什么。
