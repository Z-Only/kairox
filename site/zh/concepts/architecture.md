...
| **agent-tools**   | 工具注册表、正交的 Approval × Sandbox 策略引擎、内置工具(`shell`、`fs.read`、`fs.write`、`fs.list`、`patch`、`search`),以及 MCP-tool 适配器。                        | `ToolRegistry`、`PolicyEngine`、`ApprovalPolicy`、`SandboxPolicy`、`PolicyDecision`、`PolicyRisk`、`Tool` trait、`ToolRisk`、`McpToolAdapter`。                                    |
