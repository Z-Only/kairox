...
description: "`<memory>` 标记协议、memory 作用域、workspace 检索、基于 tiktoken budget 的 context 装配,以及 compaction。"
outline: [2, 3]
---

# Memory 与 Context

Kairox 把 memory、retrieval 和 compaction 都当作每一 turn 的一等公民。长 session 需要一种方式来记住用户偏好、项目约定和正在进行的工作,而不必每次都把它们重新粘贴一遍;它也需要能检索相关 workspace 文档,并在模型 window 满了之后丢弃旧的上下文。

本页解释 LLM 如何通过 `<memory>` 标签来"提议"记忆、memory store 如何按作用域保存这些记忆、workspace RAG 与配置的 knowledge base 如何注入检索上下文、context assembler 如何在每一 turn 内基于 tiktoken budget 构建 prompt,以及 compaction 如何在不丢失主线的前提下压缩历史。
...
查询是按 scope + workspace/session + key 来做的,还有一个 GUI 的 memory browser 用的"列出此 scope 下全部"的回退查询。持久化 memory 仍然是精确且可审视的记录;相似度检索和全文检索放在下面单独介绍的 workspace retrieval 层。

## Context 装配

每一 turn 都从头重新构建 prompt。`ContextAssembler` 就是决定"放什么进去"的组件。它拿到当前模型的 context window、最近的消息历史、相关的 memory、workspace retrieval 命中、knowledge-base 命中和新的用户 prompt,产出一个能塞进模型 token budget 之内的 `Vec<Message>`。
...
3. **Workspace retrieval。** 来自本地 `WorkspaceRagIndex` 以及已启用 `WorkspaceRetriever` 实现的命中。
4. **Knowledge base。** 按 profile 作用域启用的配置源,例如 SQLite FTS knowledge base。
5. **历史。** 来自事件流的近期消息 —— `UserMessageAdded`、被清洗过并由 `AssistantMessageCompleted` 定稿的 `ModelTokenDelta`,以及 `ToolInvocationCompleted` 的 payload。
6. **新的用户 prompt。** 永远会原文带上。
...
## Workspace retrieval 与 knowledge base

Memory record 用来保存用户或模型明确选择保留的事实。Workspace retrieval 用来处理另一类问题:"找出我没有显式保存、但当前 turn 相关的上下文"。

`agent-memory` 提供 `WorkspaceRagIndex`,这是一个 SQLite-backed chunk store,并通过可插拔的 `EmbeddingBackend` 生成向量。当前的确定性 backend 保持了本地优先和可测试性;后续本地 embedding 模型或远端 API 可以实现同一个 trait。turn 准备阶段,runtime 会查询 workspace index,并以独立的 workspace retrieval context source 注入命中结果。

配置的 knowledge base 走同一个 retrieval 边界。`[knowledge_bases.<id>]` 当前可以启用 SQLite FTS,配置模型也已经覆盖 Tantivy、Bedrock Knowledge Bases、Pinecone 和 Weaviate,方便对应服务客户端接入 `WorkspaceRetriever`。`profile_aliases` 决定哪些模型 profile 可以看到该来源。多个 retriever 同时启用时,`CompositeWorkspaceRetriever` 会在 context 装配前合并命中结果。

trace/context source 名称会把这些输入和 durable memory 区分开:已批准记录来自 `MemoryStore`,检索 chunk 则来自 workspace retrieval 或 knowledge-base source。
...
- **检索命中不是 memory。** Workspace RAG 或 knowledge-base 命中可以帮助当前 turn,但不会自动变成持久化 memory;只有模型提出 `<memory>` 标记并通过正常审批路径后,它才会进入 memory store。
