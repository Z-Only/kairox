...
<ThemeScreenshot
  ...
  caption="GUI 默认进入 workbench,项目 session、聊天、trace event、任务状态、trajectory 进度和上下文用量会一起显示。"
/>
...
完整的配置 schema——所有 provider、所有字段、所有支持的 MCP transport、`[context]` 预算章节,以及可选的 `[advisor]` 自反检查策略——都在 [Configuration](../reference/configuration)。
