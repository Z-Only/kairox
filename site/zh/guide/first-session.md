...
1. 你的消息出现在聊天区。
2. trace 区出现 `UserMessageAdded` 和 `ContextAssembled` 行。
3. 随着模型开始流式输出,trace 区出现 `ModelTokenDelta` 行,聊天区里的文本一个字符一个字符地累积出来。
...
trace 里会出现一条 `PermissionGranted` event;tool 会执行;接着出现带有结果的 `ToolInvocationStarted` 和 `ToolInvocationCompleted` event;模型继续流式输出最终答案;turn 以 `AssistantMessageCompleted` 结束。
...
每一次 turn 都会向 context 追加内容。状态栏的仪表显示当前用量占活跃 profile 的 `context_window` 的比例。当用量越过 `auto_compact_threshold`(默认 0.85)时,runtime 会触发一次自动 compaction:最老的那一层历史会被折叠成一条摘要消息。你会在 trace 里看到 `ContextCompactionStarted`、`CompactionSummary` 和 `ContextCompactionCompleted` event,仪表也会跟着回落。
...
GUI 给的是同一套 runtime,但交互方式不同:可点击的界面、常驻面板、trajectory 查看、autonomous task 控制,以及更完整的设置入口。

```bash
just tauri-dev
```

桌面窗口会打开。下面的截图展示了默认的 workbench 布局:左侧是 session 列表,中间是聊天区,右侧是 trace 加 tasks,顶部还有模型与 context 控制。
...
<ThemeScreenshot
  ...
  caption="桌面 workbench:项目 session、聊天、trace、task graph、context 用量和带 trajectory 的 runtime 状态集中在一个窗口里。"
/>
...
右侧的 trace 时间线实时更新。每个 event 都有一行;你可以搜索 trace(`/`)、按 event 类型过滤,点一行就能查看 payload。你在 TUI 里看到的 `PermissionGranted` / `ToolInvocationCompleted` / `ModelTokenDelta` 行,在这里也会出现,只是展示更丰富。

当 turn 记录了实际动作时,trajectory viewer 可以查看按顺序捕获的 action/observation step,用于 replay、debug 和 eval。工具截图及其它图片附件会保持结构化,支持 vision 的模型可以直接接收它们,而不会把整段 base64 塞爆文本 context。
...
<ThemeScreenshot
  ...
  caption="Trajectory tab 会把一次 turn 展开成按顺序记录的 tool action、input、observation、耗时和最终 outcome。"
/>
...
打开命令面板(<kbd>Ctrl/Cmd+P</kbd>),搜索 “compact context” 并执行。trace 会记录 `ContextCompactionStarted`、`CompactionSummary` 和 `ContextCompactionCompleted`;context 仪表归位;聊天记录在外观上没有变化,但最老的几条消息在内部已被一条摘要替换。
...
- 跨重启的持久化 session。
- trajectory 捕获以及 advisor 可见的 tool review event。
