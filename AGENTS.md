...
| **agent-models**  | Model provider abstraction (OpenAI-compatible, Anthropic, Ollama, Fake) with model metadata/context-window support, retry resilience, and server-side tools                                                                                                                                                          | `ModelClient` trait, `ModelRequest`, `ModelRouter`, `ModelProfile`, `ModelLimits`, `RetryConfig`, `ServerTool`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| **agent-config**  | TOML config loading, model profile discovery, API key resolution, layered user/project/local `.kairox/` discovery, skills config, instruction config                                                                                                                                                                 | `ProfileDef`, `load_from_str`, `build_router`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| **agent-gui**     | Tauri 2 + Vue 3 desktop app; Rust backend crate is `agent-gui-tauri` under `apps/agent-gui/src-tauri`                                                                                                                                                                                                                | `commands.rs`, `GuiState`, `event_forwarder.rs`, `specta.rs`, `tauri_plugin_updater`; Vue stores: `session.ts`, `sessionActions.ts`, `sessionEvents.ts`, `taskGraph.ts`, `agents.ts`, `agentSettings.ts`, `mcp/` (split by domain), `memory.ts`, `catalog.ts`, `skills.ts`, `trace.ts`, `ui.ts`, `modelProfiles.ts`, `plugins.ts`, `autonomous.ts`, `project.ts`, `workspaceUi.ts`; components: `ChatPanel.vue`, `ChatComposer.vue`, `InstructionsSettingsPane.vue`, `TaskSteps.vue`, `TaskNode.vue`, `TraceTimeline.vue`, `TraceEntry.vue`, `PermissionPrompt.vue`, `chat/{ChatPermissionItem,ChatToolCallItem,ChatCompactionItem,ChatMessageItem,ChatMonitorItem,DiffPreview}.vue`, `MemoryBrowser.vue`, `McpServerManager.vue`, `McpStatusIndicator.vue`, `SessionsSidebar.vue`, `StatusBar.vue`, `ToastContainer.vue`, `ConfirmDialog.vue`, `CommandPalette.vue`, `ContextMeter.vue`, `TrajectoryViewer.vue`, `ModelSettingsPane.vue`, `AgentSettingsPane.vue`, `AutonomousSettingsPane.vue`, `HooksSettingsPane.vue`, `PluginSettingsPane.vue`, `SkillSettingsPane.vue`, `McpSettingsPane.vue`, `marketplace/{CatalogList,CatalogCard,CatalogDetail,InstalledList,InstallProgress,RuntimeMissingHint}.vue`, `skills/{SkillCatalogDetail,SkillDiscoverCard,SkillDiscoverList,SkillSourcesSettings}.vue`, `sidebar/{ProjectSection,SessionSection}.vue`, `ui/Kx*.vue` (design-system primitives) |
...
│   │                       #   skills config, layered `.kairox/` config discovery
...
│       │   ├── views/      # WorkbenchView, MarketplaceView, settings/GeneralSettings
...
│       │   ├── components/ # ChatPanel, ChatComposer, TraceTimeline, TaskSteps, TaskNode,
...
│       │   │               #   McpSettingsPane, marketplace/*, skills/*, sidebar/*, ui/Kx*
│       │   ├── stores/     # session, sessionActions, sessionEvents, taskGraph, agents,
│       │   │               #   agentSettings, mcp/ (split by domain), memory, catalog,
│       │   │               #   skills, trace, ui, modelProfiles, plugins, autonomous,
│       │   │               #   project, workspaceUi
│       │   ├── composables/# useTauriEvents (session-filtered), useTraceStore,
│       │   │               #   useNotifications, useToast, useConfirm, useUpdater,
│       │   │               #   useMarketplace, useChatComposer, useChatStream,
│       │   │               #   useChatToolExpand, useCommandRegistry, useDiffDetect,
│       │   │               #   useDraftStore, useMentionSearch, useToolIcon,
│       │   │               #   contextFormatting
...
```bash
just test-pilot     # starts the Tauri dev app with --features pilot, then runs the scenarios
```

The recipe delegates to `scripts/run-pilot-tests.sh`, which starts `bun --filter agent-gui tauri dev --features pilot` by default and then drives the TOML scenarios against the pilot socket. The script writes per-scenario JUnit XML to `pilot-results/<name>.xml` and dumps screenshots/logs into `tauri-pilot-failures/` on failure (both directories are gitignored). On Linux you usually need `xvfb-run -a just test-pilot`; on macOS the Tauri window appears briefly during the run.
...
- **Don't forget `xvfb-run -a` when running `just test-pilot` on Linux** — the recipe launches a real Tauri window that requires a display. macOS and Windows runners use the native window server.
...
| `just test-pilot`         | Start the Tauri dev app with `--features pilot` and run the tauri-pilot scenarios |
