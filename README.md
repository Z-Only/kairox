...
- **Skills system** — native skills for reusable prompt, tool, and workflow capabilities with config-driven discovery and GUI settings management
...
- **GUI desktop app** — Tauri 2 + Vue 3 with persistent sessions, session switching, trace visualization, memory browser, permission center, skills settings, and MCP marketplace UI
...
- `crates/agent-mcp` — MCP (Model Context Protocol) client, stdio + SSE transports, server lifecycle, discovery cache, marketplace catalog (built-in + remote sources), skills system
