...
Phase 2 DAG execution with `AgentStrategy` enables multi-agent orchestration (planner / worker / reviewer). Agent settings can override model profile, permission mode, skills, and tool allowlists per role, and the runtime has been refactored into focused modules (`agent_loop`, `dag_executor`, `event_emitter`, `mcp_manager`, `memory_handler`, `permission`, `session`, `task_graph`) for maintainability.
