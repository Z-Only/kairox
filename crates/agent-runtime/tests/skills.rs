...
use agent_skills::{
    FileSkillRegistry, SkillActivation, SkillDocument, SkillId, SkillMetadata,
    SkillPermissionDeclaration, SkillRegistry, SkillRoot, SkillSource, SkillSourceKind,
};
...
#[derive(Debug)]
struct ToggleSkillRegistry {
    enabled: Arc<AtomicBool>,
    metadata: SkillMetadata,
}

impl ToggleSkillRegistry {
    fn new(enabled: Arc<AtomicBool>, metadata: SkillMetadata) -> Self {
        Self { enabled, metadata }
    }

    fn plugin_review(enabled: Arc<AtomicBool>) -> Self {
        Self::new(
            enabled,
            SkillMetadata {
...
// ── plugin skill discovery & activation integration tests ──
