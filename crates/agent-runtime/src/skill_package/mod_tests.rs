...
fn missing_npx_is_classified_as_runtime_missing() {
    let error = npx::classify_npx_spawn_error(std::io::Error::from(std::io::ErrorKind::NotFound));
...
fn parse_error_for_wrong_column_count_is_actionable() {
    ...
    let results =
        npx::parse_npx_skills_find_output("code-review\tReview code changes\tobra/superpowers\n")
