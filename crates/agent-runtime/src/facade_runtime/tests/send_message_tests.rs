...
impl TurnExecutor for BlockingTurnExecutor {
    async fn execute_turn(
        ...
        let release = self
            ...
            .await
            .take()
            .expect("blocking turn should be released once");
        let _ = release.await;
        Ok(())
    }
}
...
#[tokio::test]
async fn send_message_strict_queues_same_session_turn_when_actor_turn_running() {
    let store = SqliteEventStore::in_memory().await.unwrap();
    let (started_tx, started_rx) = oneshot::channel();
    let (release_tx, release_rx) = oneshot::channel();
    let stream_calls = Arc::new(AtomicUsize::new(0));
    ...
    let second = tokio::spawn(async move {
        second_runtime
            .send_message_strict(SendMessageRequest {
                workspace_id: second_workspace_id,
                session_id: second_session_id,
                content: "second".into(),
                display_content: None,
...
async fn send_message_if_idle_rejects_running_session() {
    ...
    let result = runtime
        .send_message_if_idle(SendMessageRequest {
            workspace_id: workspace.workspace_id,
            session_id: session_id.clone(),
            content: "second".into(),
            display_content: None,
    ...
    match result {
        Err(agent_core::CoreError::SessionBusy {
            session_id: id,
            reason,
        }) => {
            assert_eq!(id, session_id.to_string());
            assert!(reason.contains("running"), "unexpected reason: {reason}");
        }
        other => panic!("expected SessionBusy, got {other:?}"),
    }
    assert_eq!(stream_calls.load(Ordering::SeqCst), 1);
...
#[tokio::test]
async fn send_message_queued_queues_same_session_turn_when_actor_turn_running() {
    let store = SqliteEventStore::in_memory().await.unwrap();
    let (started_tx, started_rx) = oneshot::channel();
    let (release_tx, release_rx) = oneshot::channel();
    let stream_calls = Arc::new(AtomicUsize::new(0));
    ...
    let second = tokio::spawn(async move {
        second_runtime
            .send_message_queued(SendMessageRequest {
                workspace_id: second_workspace_id,
                session_id: second_session_id,
                content: "second".into(),
                display_content: None,
...
#[tokio::test]
async fn send_message_strict_rejects_cancelling_session() {
    let store = SqliteEventStore::in_memory().await.unwrap();
    let (started_tx, started_rx) = oneshot::channel();
    let (release_tx, release_rx) = oneshot::channel();
    let executor = Arc::new(BlockingTurnExecutor::new(started_tx, release_rx));
    ...
    runtime
        .session_execution
        .cancel_session(&session_id, "test cancellation".into())
        .await
        .unwrap();
    ...
    match result {
        Err(agent_core::CoreError::SessionBusy {
            ...
            assert_eq!(id, session_id.to_string());
            assert!(reason.contains("cancelling"));
        }
        other => panic!("expected SessionBusy, got {other:?}"),
    }
